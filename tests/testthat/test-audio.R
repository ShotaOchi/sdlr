test_that("audio",
{
  ...
  expect(audio$getstatus() == "playing", "audio is not playing.")
  expect_no_error(audio$queue())
  expect_no_error(audio$clear())
  expect_no_error(audio$pause())
  expect(audio$getstatus() == "paused", "audio is not paused.")
  expect_no_error(audio$unpause())
  expect(audio$getstatus() == "playing", "audio is not playing.")
  expect_no_error(print(audio))
  expect_no_error(rm(audio))
  expect_no_error(gc())
})
